 ...
 * @returns {DataType[][]}
 */
function array2D(columns, rows, fillValue = null){
  const array = [];
 ...
 * @param {DataType} fillValue
 */
function createHexArray(width, height, fillValue = 1, emptyValue = 0){
  const array = array2D(width, height, emptyValue);
 ...
 */
function overrideArray2D(source, array, offsetX = 0, offsetY = 0, nullValue = 0){
  const start = {
 ...
 * @param {DataType} fillValue
 */
function markBorderArray2D(array, fillValue, nullValue, borderX = 1, borderY){
